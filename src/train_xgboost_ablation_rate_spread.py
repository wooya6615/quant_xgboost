"""
금리 스프레드(국고채 10년-3년) feature 추가 전/후 성능을 비교하는 ablation 스크립트
(train_xgboost_ablation_fx.py를 RATE_SPREAD용으로 리네이밍 -- 구조/로직은 100% 동일, FX -> RATE)

핵심 설계:
    같은 데이터, 같은 fold 구성, 같은 하이퍼파라미터로
    BASE(가격 feature 13개) vs BASE + RATE_SPREAD(금리스프레드 feature 4개 추가)만 비교함.
    -- 그래야 성능 차이가 순수하게 "금리스프레드 feature 추가 효과"인지 확인 가능.
    (train_xgboost_wfo.py의 walk-forward 구조를 그대로 재사용)

run_horizon_sweep(): feature_engineering_rate_spread.py가 만든
    {ticker_krx}_features_with_rate_spread_h{horizon}.csv들을 순서대로 읽어서
    각 horizon마다 멀티시드 ablation을 돌리고, horizon별 COMBINED-BASE AUC 차이를
    한 표로 모아줌 -- "금리스프레드 신호가 며칠 후에 가장 잘 반영되는지" 한눈에 비교 가능.

사용법 (레포 루트에서):
    python src/train_xgboost_ablation_rate_spread.py

전제:
    feature_engineering_rate_spread.py를 먼저 실행해서
    {ticker_krx}_features_with_rate_spread_h{horizon}.csv 들이 만들어져 있어야 함.
"""
...
FEATURE_COLS_RATE = [
    "rate_spread_10y3y", "rate_spread_chg_5d",
    "rate_spread_chg_20d", "rate_spread_deviation_ma20",
]

FEATURE_COLS_COMBINED = FEATURE_COLS_BASE + FEATURE_COLS_RATE

FEATURE_COLS_RATE_ONLY = FEATURE_COLS_RATE  # 가격 feature 없이 금리스프레드 feature 4개만
...
def load_dataset(ticker_krx: str = "064350", horizon: int = DEFAULT_HORIZON) -> pd.DataFrame:
    path = DATA_DIR / f"{ticker_krx}_features_with_rate_spread_h{horizon}.csv"
...
def compare_feature_sets(df: pd.DataFrame, horizon: int, random_state=42, **wfo_kwargs):
    ...
    rate_fold_df, rate_importance = run_walk_forward(
        df, FEATURE_COLS_RATE_ONLY, embargo=horizon, random_state=random_state, **wfo_kwargs)
...
def run_multi_seed(df: pd.DataFrame, horizon: int, seeds=(42, 1, 7, 123, 2024), **wfo_kwargs):
    ...
    for seed in seeds:
        ...
        rows.append({
            ...
            "RATE_auc": result["summary"].loc["auc", "RATE_ONLY"],
...
# ------------------------------------------------------------------
# 6. Horizon 스윕 -- 여러 horizon에 대해 멀티시드 ablation을 자동 반복
#    "금리스프레드 신호가 며칠 후에 가장 잘 반영되는가"를 horizon별 AUC 차이로 비교
# ------------------------------------------------------------------
def run_horizon_sweep(ticker_krx: str, horizons: list = None, seeds=(42, 1, 7, 123, 2024), **wfo_kwargs):
    ...
    for horizon in horizons:
        ...
        try:
            ...
            print(f"  {ticker_krx}_features_with_rate_spread_h{horizon}.csv 없음 -- "
                  f"feature_engineering_rate_spread.py를 먼저 실행하세요.")
            continue
        ...
        horizon_rows.append({
            ...
            "RATE_auc_mean": seed_df["RATE_auc"].mean(),
        })
...
if __name__ == "__main__":
    TICKER_KRX = "064350"  # 현대로템 -- feature_engineering_rate_spread.py와 동일 종목으로 시작

    # (A) 단일 horizon(기본값) 빠른 확인
    print("=" * 60)
    ...
    print(f"\nRATE_ONLY feature importance:\n{result_default['rate_importance']}")

    # (B) 5-seed 검증 (기본 horizon)
    print("\n\n" + "=" * 60)
    ...
    if not horizon_summary.empty:
        ...
        print("사용 가능한 horizon 데이터셋이 없음 -- feature_engineering_rate_spread.py를 먼저 실행하세요.")