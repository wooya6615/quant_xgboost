"""
COMBINED(BASE+FX) 전략의 거래를 연도별로 쪼개서 확인:
    - 총 복리수익의 대부분이 특정 연도 하나에 몰려있는지
    - 그 연도가 실제로 강한 상승장(벤치마크 대비)이었는지
    - 현대로템(+143.6%p)과 대한제강(-36.5%p)의 결과가 반대로 나온 게
      "국면 우연"인지, 아니면 일관된 신호 차이인지 확인하기 위함

사용법:
    python analyze_signal_concentration_fx.py

전제:
    backtest_comparison_fx.py, train_xgboost_ablation_fx.py와 같은 폴더에 있어야 함.
"""

import pandas as pd
import numpy as np

from backtest_comparison_fx import generate_trades, HORIZON
from train_xgboost_ablation_fx import load_dataset, FEATURE_COLS_COMBINED


def analyze_by_year(ticker_krx: str, horizon: int = HORIZON, threshold: float = 0.55):
    ...
    # 중도 판정: 특정 연도 하나가 log-return 기여도의 50% 이상이면 "집중"으로 표시
...
if __name__ == "__main__":
    ...
    print("# 현대로템 (064350)")
    print("#" * 70)
    analyze_by_year("064350")

    print("\n\n" + "#" * 70)
    print("# 대한제강 (084010)")
    print("#" * 70)
    analyze_by_year("084010")