...
| **3번째 종목: 한전기술(052690, 저유동성) 교차검증** | horizon=20 VALUATION_ONLY AUC **0.685** (당시 최고), 5/5 시드 일관. 2025년 제외해도 COMBINED 172.0% vs Buy&Hold 128.9% -- Buy&Hold 재차 상회. 단, 최대 기여 연도는 2025년이 아니라 **2021년(원전 테마 급등, +371%)** -- 서로 다른 두 개의 개별 강세장(2021, 2025)에서 각각 작동, 특정 해 하나의 우연이 아님을 시사 |
| **4번째 종목: 모트렉스(118990, 코스닥/초저유동성) 교차검증** | horizon=20 VALUATION_ONLY AUC **0.693** (전체 최고), COMBINED-BASE +0.063(전체 최고) -- ablation은 저유동성 정도에 비례해서 계속 강해짐. 그러나 최대 기여 연도가 **2020년(코로나 이후 급등, +180%) 하나에 log-return 기여도 74.9%로 집중** -- 지금까지 중 가장 심한 편중. 그 2020년을 포함한 백테스트(2025만 제외)에서도 **COMBINED 151.6%가 Buy&Hold 211.8%에 못 미침** -- 저유동성 종목인데도 실전 우위 재현 실패 |
| **5번째 종목: 대한제강(001430, 저유동성) 교차검증** | ablation부터 흔들림 -- horizon=5에서만 5/5 시드 전부 COMBINED가 BASE보다 나쁨(-0.0153), 다른 horizon은 개선(+0.005~+0.047)이라 지금까지의 "horizon 길수록 단조 증가" 패턴이 처음 깨짐. 백테스트는 최악: horizon=10 COMBINED **-68.0%**(BASE도 -8.2%로 손실), Buy&Hold(+145.2%)와 격차 극심. 국면 집중도도 특정 연도 편중이 아니라 여러 해에 걸쳐 골고루 손실이 누적된 구조 -- "이 전략이 이 종목엔 구조적으로 안 맞는다"에 가까움 |

**결론**:
1. Ablation 수준(AUC)에서는 밸류에이션 feature가 5종목 중 4종목까지는 일관되게 유의미한 신호를
   보였으나, 대한제강에서 처음으로 horizon=5 구간의 방향이 뒤집히는 예외가 나옴 -- "저유동성이면
   AUC 신호가 항상 깔끔하다"고 일반화할 수는 없음
2. 그 신호를 거래 전략(threshold+순차진입) 손익으로 전환하면 종목별 편차가 훨씬 큼 --
   AUC 개선이 실전 손익 우위로 자동 전환되지 않는다는 기존 결론을 재확인. 특히 대한제강은
   BASE/COMBINED 둘 다 절대 손실을 내며 Buy&Hold와 큰 격차로 뒤짐 -- 저유동성이라고 해서
   "밑져야 본전"조차 보장되지 않음
3. **저유동성 4종목 중 백테스트가 Buy&Hold를 이긴 건 2개(현대로템, 한전기술)뿐이고, 나머지 2개
   (모트렉스, 대한제강)는 실패함.** 즉 유동성은 통계적 신호(AUC)와는 어느 정도 상관관계를 보이지만,
   실전 손익 우위를 보장하는 충분조건이 아니라 종목별 특수성(업종, 추세 방향, 변동성 구조 등)이
   더 크게 작용하는 것으로 보임
4. 종합하면 밸류에이션 feature는 "가격 feature와 독립적인 통계적 정보를 담고 있다"는 근거는
   충분히 쌓였지만, 이를 범용적인 실전 트레이딩 전략(고정 threshold, 순차진입)으로 그대로
   수익화하기는 어렵고 종목별 맞춤 검증이 필요하다는 게 최종적으로 더 정확한 결론

**5종목 종합 비교표**

| 종목 | 유동성 | AUC 패턴 (horizon 스윕) | horizon=10 백테스트 |
|---|---|---|---|
| 현대로템 (064350) | 낮음 | 단조 증가, 깔끔 | ✅ Buy&Hold 상회 |
| 한전기술 (052690) | 낮음 | 단조 증가, 깔끔 | ✅ Buy&Hold 상회 |
| 모트렉스 (118990) | 매우 낮음 | 단조 증가, 전체 최고 | ❌ Buy&Hold 못 미침 |
| 대한제강 (001430) | 낮음 | h5 구간 역전, 불안정 | ❌ 절대 손실(-68.0%) |
| 삼성전자 (005930) | 높음 | 상대적으로 약함 | ❌ Buy&Hold 못 미침 |
...
- "저유동성 종목"을 고를 땐 시가총액이 작다고 거래대금도 낮다는 보장이 없음 (예: 후성은 시총이
  현대로템의 1/13인데 거래대금은 오히려 더 높음 -- 테마성 회전율 때문). `check_liquidity.py`로
  `get_market_cap_by_date`/`get_market_trading_value_by_date`를 직접 조회해서 숫자로 확인할 것.
  "저유동성"과 "저변동성"도 다른 개념이니 혼동하지 말 것 -- 현대로템은 2025년 +261% 급등할 만큼
  변동성은 컸지만, 회전율(거래대금/시총)은 낮은 저유동성 종목이었음.
- 코스닥 종목은 yfinance 티커 접미사가 `.KS`(코스피)가 아니라 `.KQ`임 -- 다르게 넣으면 조용히
  1행짜리 빈 데이터가 나올 뿐 에러가 안 나서 원인 파악이 늦어질 수 있음 (모트렉스 실험에서 실제로 발생).
- "AUC가 개선됐다"와 "그 종목에서 실전 백테스트가 Buy&Hold를 이겼다"는 별개 결론임 -- 4종목 밸류에이션
  검증에서 AUC는 4/4 종목 모두 저유동성일수록 강해지는 패턴이 나왔지만, 실전 백테스트 우위는 2/4
  종목에서만 재현됐음 (모트렉스는 AUC가 가장 높았는데도 백테스트에서 짐). 둘을 같은 결론으로
  섞어서 보고하지 말 것.
- 상장 이력이 짧은 종목(모트렉스: 2018년 상장)은 fold 개수가 자동으로 줄어듦 -- 결과를 다른 종목과
  비교할 때 fold 수 차이도 같이 명시할 것.
